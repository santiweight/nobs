...
class NobsContribution extends Disposable {
	...
	private _attachTerminalToContainer(tabId: string, container: HTMLElement, instance: ITerminalInstance, panel: 'agent' | 'output'): void {
		...
		const activeTabEvent = panel === 'agent'
			? this._nobsCenterService.onDidChangeActiveAgentTab
			: this._nobsCenterService.onDidChangeActiveOutputTab;

		this._register(activeTabEvent(activeId => {
	...
	private async _createOutputTerminal(tabId: string, container: HTMLElement): Promise<void> {
		...
		this._attachTerminalToContainer(tabId, container, instance, 'output');
	}
	...
	private async _createBrowser(tabId: string, container: HTMLElement): Promise<void> {
		...
		chrome.style.cssText = 'display:flex;align-items:center;height:32px;background:var(--vscode-editorGroupHeader-tabsBackground);border-bottom:1px solid var(--vscode-panel-border);padding:0 8px;gap:4px;flex-shrink:0;';
		...
		urlBar.style.cssText = 'flex:1;height:24px;background:var(--vscode-input-background);border:1px solid var(--vscode-input-border, var(--vscode-panel-border));border-radius:4px;color:var(--vscode-input-foreground);padding:0 8px;font-size:12px;outline:none;font-family:inherit;';
	...
	private _createNavButton(label: string): HTMLElement {
		...
		btn.style.cssText = 'width:24px;height:24px;background:none;border:none;color:var(--vscode-descriptionForeground);cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center;border-radius:4px;padding:0;';
		btn.addEventListener('mouseenter', () => { btn.style.background = 'var(--vscode-toolbar-hoverBackground)'; btn.style.color = 'var(--vscode-foreground)'; });
		btn.addEventListener('mouseleave', () => { btn.style.background = 'none'; btn.style.color = 'var(--vscode-descriptionForeground)'; });
		return btn;
	}
}
